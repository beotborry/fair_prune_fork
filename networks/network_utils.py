...
class Mask(nn.Module):
    def __init__(self, init_value=[1], planes=None, for_fc=False):
        super().__init__()
        self.planes = planes
        self.for_fc = for_fc
        self.mask = Parameter(torch.Tensor(init_value))

    def forward(self, input):
        weight = self.mask

        if self.planes is not None:
            weight = self.mask[None, :, None, None]
        elif self.for_fc:
            weight = self.mask[None, :]
