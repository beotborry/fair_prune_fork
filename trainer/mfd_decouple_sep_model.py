    ...
    def _train_epoch(self, epoch, train_loader, model, teacher, distiller):
        ...
        for i, data in enumerate(train_loader, 1):
            ...
            for g in range(self.num_groups):
                if (groups == g).sum() == 0:
                    continue
                g_inputs = inputs[groups==g].to((g%2)+1)
                g_labels = labels[groups==g].to((g%2)+1)
                ...
                f_s = g_outputs
                mmd_loss = distiller.forward(f_s, f_t.to((g%2)+1), t_labels=labels.to((g%2)+1), s_labels=g_labels)
...
class MMDLoss(nn.Module):
    ...
    def forward(self, f_s, f_t, s_labels, t_labels):
        if self.kernel == 'poly':
        ...
        for c in range(self.num_classes):
            if len(teacher[t_labels == c]) == 0 or len(student[s_labels == c]) == 0:
                continue

            K_TS, _ = self.pdist(teacher[t_labels == c], student[(s_labels == c)],
                                 sigma_base=self.sigma, sigma_avg=sigma_avg,  kernel=self.kernel)
            K_SS, _ = self.pdist(student[(s_labels == c)], student[(s_labels == c)],
                                 sigma_base=self.sigma, sigma_avg=sigma_avg, kernel=self.kernel)
            K_TT, _ = self.pdist(teacher[t_labels == c], teacher[t_labels == c], sigma_base=self.sigma,
