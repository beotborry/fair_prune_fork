...
def round_to_even(arr):
    return np.int_(np.round(arr/2)*2)
...
class ShuffleNetV2(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, num_classes=1000, inverted_residual=InvertedResidual,
        ...
        stages_out_channels = list(round_to_even(np.int_(np.array(stages_out_channels) * resize_ratio))) \
            if resize_ratio != 1.0 else stages_out_channels
